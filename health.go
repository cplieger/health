...
func (m *Marker) Set(ok bool) {
	...
	if ok {
		if err := writeMarker(m.path); err != nil {
			m.warnFailure("failed to create health marker", err)
			return
		}
		if recovered := m.recordState(true); changed || recovered {
	...
	if err := os.Remove(m.path); err != nil && !errors.Is(err, fs.ErrNotExist) {
		m.warnFailure("failed to remove health marker", err)
		return
	}
	if recovered := m.recordState(false); changed || recovered {
